#include "accel.h"
#include "xil_io.h"
inline void Wait_AP_Done(){//apdone 为1时触发，再置0
	while(ap_done==0);
	ap_done=0;
	return;
}
inline void Send_DMA_Task(uint32_t bufaddr,uint32_t buflen){
	...
	Xil_Out32(X_AXI_DMA_SEND_ADDR_0,bufaddr);//X_AXI_DMA_SEND_ADDR_0=DMA地址+RX偏移地址（0x0）+RX基址，将传输内容的起始地址写入AXI设备
	Xil_Out32(X_AXI_DMA_SEND_ADDR_2,buflen);//将传输长度写入AXI设备中
	return;
}
...
void Set_Next_IFM_ADDR_LEN(){
	if(iter_ofm_post!=ofm_batch-1||iter_div_post!=fm_div_cnt-1||iter_ifm_post!=ifm_batch-1){
		ifm_send_task_enable=1;//允许运行input特征图的标志位
...
void Initiate(
	...
	iter_ofm_post=0;
	iter_div_post=0;
	//xil_printf("Initiate finish\n\r");
	//xil_printf("ofm: %x ifm: %x  \n\r",ofm,ifm);
	//xil_printf("ofm_batch: %x ifm_batch: %x sel:%x \n\r",ofm_batch,ifm_batch,sel);
	//xil_printf("ifm_ddr_base_addr: %x \n\r",ifm_ddr_base_addr);
	return;
}
void out_OFM(uint8_t *ofmbuf,int fmcol,int fm_row,int fmch){
		...
		f_open(&fil,"OFMS1.bin",FA_CREATE_ALWAYS|FA_WRITE);
		f_write(&fil,ofmbuf,fmcol*fm_row*fmch,&wr_tot);
		f_close(&fil);
		xil_printf("Finish Save_OFM!write %d bytes\n\r",wr_tot);
		return;
	}
void YOLOV3_TINY(){
//	XTime_GetTime(&tCur);
//	xil_printf("enter yolo tiny\n\r");
...
//	out_OFM((uint8_t)*ofm_ddr_base_addr_in_list[MAX_TASK-1],418,258,255);
	return;
}
...
//					xil_printf("addr result: %d \n\r",Addr_Calc(t*(CLASS+5)+4,i,j));
//					xil_printf("curr_p:%d trec:%d irec:%d jrec:%d  \n\r ",curr_p,trec,irec,jrec);
//					xil_printf("decect result: x:%d y:%d w:%d h:%d category:%d \n\r ",(int)x,(int)y,(int)w,(int)h,category);
				}
			}
		}
	}
...
//	ydown=(int)(y+h)/2;
	...
	//xil_printf("decect result: x:%d y:%d w:%d h:%d category:%d \n\r ",(int)x/2,(int)y/2,(int)w/2,(int)h/2,category);
	//xil_printf("xleft :%d xright:%d ytop:%d ydown:%d \n\r ",xleft,xright,ytop,ydown);
	//xil_printf("prob: %d.%.08d\n\r", (int)(prob), (int)(fabs(prob)*100000000)%100000000);
	return;
}
